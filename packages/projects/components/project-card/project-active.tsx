...
export const ProjectActiveCapsule = (props: ProjectActiveCapsuleProps) => {
    ...
    }, [pathName, props.projectId]);

    if (active) {
        return (
            <span className="h-5 w-[3px] bg-white-100 rounded-full shadow-[0px_0px_8px_rgba(255,255,255,0.5)] absolute left-[-8px]"></span>
        );
    } else {
        return null;
    }
...
export const ProjectActiveLinkTree = (props: ProjectActiveLinkTreeProps) => {
    ...
    if (active) {
        return (
            <div className="flex flex-col pl-[1.45rem]">
                {pageLinks.map((link) => (
                    <Link
                        href={`/project/${props.projectId}/${link.slug}`}
                        key={link.slug}
                    >
                        <div className="flex items-end gap-2.5">
                            ...
                            <div className="flex items-center h-fit gap-2 hover:text-white-24">
                                <i
                                    className={`fi fi-sr-${link.icon} h-3.5 flex items-center`}
                                ></i>
                                <span
                                    className={`h-fit font-bold font-body-b cursor-pointer font-display hover:text-white-24 ${
                                        pathName.includes(link.slug)
                                            ? 'text-white-100 hover:text-white-72'
                                            : 'text-white-48'
                                    }`}
                                >
                                    {link.name}
