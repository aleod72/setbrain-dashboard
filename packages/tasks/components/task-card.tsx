    ...
    return (
        <div className="flex flex-col justify-between border-2 rounded-2xl border-darkgrey-48 bg-darkgrey-100 py-3 px-4 w-full min-w-[300px] max-w-[345px] h-[13.5rem] cursor-pointer hover:border-darkgrey-86" onClick={() => router.push(pathname +  '/' + task?.id)}>
            <div className="flex justify-between items-end">
                ...
                <span className="h-6">
                    <ProfilePictureList
                        ids={task.assigned_users ?? []}
                    ></ProfilePictureList>
                </span>
            </div>
            <div className="flex flex-col gap-2 mt-2">
                <div className="flex gap-1 flex-col">
                    <h1 className="font-bold text-body-lb text-white-100 w-52">
                        {task.title}
                    </h1>
                    <span className="text-body-s text-white-48">
                        Pour le {dayjs(task.end_at).format('DD/MM/YY')}
                    </span>
                </div>
                <ProgressBar
                    progress={task.progress * 100}
                    max={100}
                    intent={getColorByUrgency(task.end_at ?? '', task.progress)}
                ></ProgressBar>
            </div>
            <div className="flex justify-between mt-3">
                <div className="flex gap-3">
                    <span className="text-pretitle flex items-center gap-1">
                        <i className="fi fi-rr-comment-alt text-white-100 text-body-l"></i>{' '}
                        {task.comments?.length ?? 0}
                    </span>
                    <span className="text-pretitle flex items-center gap-1">
                        <i className="fi fi-rr-clip text-white-100 text-body-l"></i>{' '}
                        {task.joined_files?.length ?? 0}
                    </span>
                </div>
                <a className="font-bold text-white-100 text-body-b">
                    Voir plus
                </a>
            </div>
        </div>
