...
export const Message = ({ message }: MessageProps) => {
    ...
    return <div className={`flex w-full ${isLoggedUser ? 'justify-end': 'justify-start'}`}>
        <div className={`flex w-fit items-end gap-2 ${isLoggedUser && 'flex-row-reverse'}`}>
            <div className='w-6 h-6 relative'>
                {user != undefined && <ProfilePicture id={user.id}></ProfilePicture>}
            </div>
            <div className={`flex flex-col bg-darkgrey-48 p-2 rounded-t-xl w-48 ${isLoggedUser ? 'rounded-bl-xl': 'rounded-br-xl'}`}>
                <span className='text-body-s text-white-48'>{user?.firstname} {user?.lastname}</span>
                <p className='text-body-s text-white-100'>{message.message}</p>
            </div>
        </div>
    </div>;
};