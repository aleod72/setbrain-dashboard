...
const GoogleAppsToIcon: { [key: string]: string } = {
...
export const FileItem = ({ file, onDoubleClick }: FileItemProps) => {
    ...
    const iconName = isFolder
        ...
        ? GoogleAppsToIcon[file.mimeType] || 'fi-sr-file'
        : FileExtensionToIcon[file.fileExtension || ''] || 'fi-sr-file';
    const backgroundColor = isFolder
        ? 'bg-black-72'
        : 'border-2 border-darkgrey-100';
        ...
        >
            <div className="flex gap-4 items-center">
                ...
                <div className="flex flex-col gap-[2px]">
                    <span className="text-sm text-ellipsis w-40 overflow-hidden whitespace-nowrap">
                        {file.name}
                    </span>
                    <span className="text-xs mr-2 text-white-48">
                        modifié le {dayjs(file.modifiedTime).format('DD/MM/YY')}
                    </span>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <span className="text-xs mr-2">
                    {file.size &&
                        ...
                        prettyBytes(Number.parseInt(file.size))}
                </span>
                <div className="h-6">
...
export const FileItemSkeleton = () => {
    return (
        <div className="flex justify-between rounded-xl py-2 px-4 items-center border-2 border-darkgrey-100">
            <div className="flex gap-4 items-center">
                <Skeleton
                    width={24}
                    height={24}
